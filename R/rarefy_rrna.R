...
#' @param rRNA A dataframe with two variables: "ID" is the OTU id matched by names in x and "Copy" is the copy number
...
rarefy_rrna <- function(x, reads, rRNA, seed=NULL){
  if(class(x) == "phyloseq") y <- rarefy_rrna.phyloseq(x,reads,rRNA,seed) else y <- rarefy_rrna.matrix(x,reads,rRNA,seed)
  return(y)
}
